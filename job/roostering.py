...
for compare in duplicate_student:
	print compare + " : " + str(len(duplicate_student[compare]))
...
for vak in vakken:
	duplicate_student.setdefault(vak, {})
...
for vak in duplicate_student:
	for ander_vak in duplicate_student[vak]:
		if vak is not ander_vak:
			fid.wl(vak + "," + ander_vak + "," + str(len(duplicate_student[vak][ander_vak])))
fid.close()
"""
rooster.close()
info.close()