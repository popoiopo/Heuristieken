...
# Evaluatie Weekverdeling en x-1,x-2
...
for vak in vak_dict:
	...
	for i in range(0, len(vak_dict[vak])):
		buffer1.append(int(vak_dict[vak][i][2]))
	groepen = max(buffer1)
	...
	if(aantal_contacturen > 1):
		...
		# maak weekrooster voor iedere groep
			for j in range(1,(groepen + 1)):
				...
				for k in range(len(vak_dict[vak])):
					if(vak_dict[vak][k][2] == "0"):
						weekrooster_enkel.append(vak_dict[vak][k][:2])
					check = int(vak_dict[vak][k][2])
					if(check == j):
						weekrooster_enkel.append(vak_dict[vak][k][:2])
				#score subgroep
			...
			weekrooster_enkel = []
			for l in range(len(vak_dict[vak])):
				weekrooster_enkel.append(vak_dict[vak][l][:2])
			print(weekrooster_enkel)
