...
max_faults_in_recombination = 30 #maximum recombination faults is n
population_size_per_generation = int(2*best_scores_maxsize) #laat populatie groeien tot deze size voordat selectie wordt toegepast
...
def check_geldigheid_rooster(table, gene_pool, parent1, parent2):
	...
	if (len(check_all_subjects)) < max_faults_in_recombination and (len(check_all_subjects)) > 10:
	...
	elif (len(check_all_subjects)) < 5:
		for i in range(0,n_mutations):
			print(i)
			mutation(table, days_in_week, time_frames , classroom_info)
#weg!		check_geldigheid_rooster(table, gene_pool, parent1, parent2)
...
def mutation(ttable, week, timeslots , classroom_info):
	...
	room = random.choice(list(classroom_info.keys()))
	if not bool(timetable[day][time][room]):
		mutation(ttable, week, timeslots , classroom_info, mutaties)
#		timetable[day][time][room][course] = gro_stu_dat[course]
	else:
		subject = list(table[day][timeslot][classroom].keys())[0]
...
###	while i < (population_size_per_generation):
