...
def take_best_scores2(scores, passed_scores, table, x, max_size):
	print(scores.keys())
	key = sorted(scores.keys())[0]
	min_value_1 = scores.pop(key)
	Ttable = {}
	if passed_scores[x] > passed_scores[(x-1)]:
		Ttable = copy.deepcopy(table)
#		print(passed_scores[x])
		if passed_scores[x] in list(scores.keys()):
#			print('oeps')
			check = unique_score(score_total, score_total[i])
			passed_scores[x] = check
#		print(Ttable)
...
while i < 10:
	...
	##eerste solo uitpluizen en dan pas invullen in dict]
...
#		t_table = copy.deepcopy(best_scores_hillcl[key])
		...
		best_scores_hillcl[key] = take_best_scores2(best_scores_hillcl, score_total_hillcl[key], table, i, best_scores_maxsize)
	i += 1
	print(i)
