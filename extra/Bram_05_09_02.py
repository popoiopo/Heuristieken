...
#	subject_name = list(subject.keys())[0]
		print(subject_name ,'in function delete_random_subject')
		return subject_name
...
best_scores_random = {}
best_scores_hillcl = {}
best_scores_maxsize = 6
#best_scores = {-1001: 'lala', -100: 'haha', -10000: 'hihi'}
best_scores_random = {-1001: 'lala'}
for i in range(0,20):
...
while i < 100:
	...
	##zet timetable niet in dict!!!
	##eerste solo uit[pluizen en dan pas invullen in dict]
	for j in range(0,best_scores_maxsize):
		key = str(ascii_lowercase[j])
		t_table = copy.deepcopy(best_scores_hillcl[key])
		print(key)
		random_subject = deepcopy(delete_random_subject(best_scores_hillcl[key]))
		print(random_subject, 'in main')
		best_scores_hillcl[key] = scheduling(random_subject, best_scores_hillcl[key], group_student_database, days_in_week, time_frames, classroom_info)
		time_table_points(best_scores_hillcl[key], score_total_hillcl[key], score_double_students_hillcl[key],score_classrooms_hillcl[key], score_ditribution_in_week_hillcl[key])
	i += 1
#excel_schedule(time_table, days_in_week, time_frames, classroom_info)
