...
subject_student_database1["Calculus_2"] = "" 
print subject_student_database1
...
#					print(group)
					...
					x += check
					print(len(group_student_database[vak_naam](j)))
			##-------------------------
	...
	for key1, value1 in group_student_database.items():
		for key2, value2 in subject_student_database.items():
			...
			if check_key1 == key2 and key1[:2] == "hc":
				#print(key1)
				#print(key2)
				if len(group_student_database[key1]) == (subject_student_number[key2]):
					#print('check')
					haha=2
				else:
					print ('error')
